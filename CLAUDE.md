...
- **`packages/solver`** — the rebalancing engine. Pure TypeScript: no DOM, no `fetch`, no filesystem access, no network calls. Deterministic — same input always produces the same output. Its public exports are `rebalance()`, `validateScenario()`, `DEFAULT_TOLERANCE_BPS`, and the domain types in `src/types.ts`; nothing else in `src/` should be imported from outside the package.
  ...
  - `src/allocate.lp.ts` — the **default** engine (`options.optimizer`, default `"lp"`; `"greedy"` selects `allocate.ts`), using YALPS (pure JS, synchronous — the async/WASM LP libraries would break `rebalance()`'s sync purity). Lexicographic objectives (deviation → total sells → taxable sells → tax-preferred placement), each pinned as a constraint before the next solve. It was made the default because greedy only ever sells globally-overweight classes and so cannot *relocate* a class between accounts, stranding gaps on restricted-menu portfolios (see the "restricted fund menus" tests). `minTradeCents` is honored by iterative refinement: ban selling any position whose sell lands below the floor, re-solve, repeat. Simplex floats are rounded back to integer cents by snapping (near-)untouched positions to their current value (so noise never fabricates a one-cent trade) plus a per-account largest-remainder repair: account totals stay exact; class totals can carry < 1 cent of noise per account, which the property tests carry as explicit slack. The tolerance band is an *eligibility* test computed from the inputs (same as greedy): beyond-band classes are pulled to their exact target, within-band classes are frozen against selling. `src/allocate.property.test.ts` brute-forces small problems to hold *both* implementations to the same optimality bar, plus a differential property (lp never worse than greedy).
